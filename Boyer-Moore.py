...
def fundamental_preprocess(S):
    """
    Returns Z, the Fundamental Preprocessing of S. Z algorithm
    """
    if len(S) == 0: # empty string
        return []
    if len(S) == 1: # single-character string
        return [1]
    z = [0 for x in S]
    z[0] = len(S)
...
def bad_character_table(S):
    """
    Generates R for S, which is an array indexed by the position of some character c.This is used for a constant-time lookup
...
def full_shift_table(S):
    ...
    string search algorithm.Condition to check if the entire string is repeated again.
    """
    F = [0 for c in S]
